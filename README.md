...
6. **アクセス修飾子の刷新と記述の必須化**:
    *   初学者にとって直感的なアクセス修飾子（`public`, `private`, `family`, `package`）を導入します。
...
9.  **`cls` / `ins` / `new` キーワードによる明示的なメンバー宣言**:
...
11. **インスタンス経由でのクラスメンバー（`cls`）アクセス禁止**:
...
PyJaでは、クラス（およびインターフェース、enum）の宣言行、および `<field>`, `<const>`, `<method>` セクション内のすべての定義行において、以下のアクセス修飾子のいずれかを行頭（インデントを除く）に明示することが必須です。

| PyJaキーワード | 意味 | Javaへの変換 |
|---|---|---|
| `public` | どこからでもアクセス可能 | `public` |
| `private` | 同じクラス内からのみアクセス可能 | `private` |
| `family` | 「同じ家内（同一パッケージ内）」および「家族（継承関係）」からアクセス可能（Javaの `protected` 相当） | `protected` |
| `package` | 「同じ家内（同一パッケージ内）」からのみアクセス可能 | 削除（Javaのパッケージプライベート） |
...
*   `Syain` はパッケージ内でのみ利用する内部クラスとするため `package class` としています。
